...
class Patient(object):
    ...
    def __init__(self, viruses, maxPop):
        ...
        self.maxPop = maxPop
        self.density = len(self.viruses)/self.maxPop
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for timestep in range(300):
            newpop = patient.update()
            poptimes[timestep] += float(newpop)
    populations = [x/numTrials for x in poptimes]
    pylab.plot(range(300), populations)
            ...
            for key in self.resistances:
                if self.resistances[key]:
                    ...
                    if random.random() <= self.mutProb:
                        ...
                        newdict[key] = False
            if random.random() <= (self.getMaxBirthProb()*(1-popDensity)):
                return ResistantVirus(self.getMaxBirthProb(), self.getClearProb(), newdict, self.getMutProb())
            else:
                raise NoChildException
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in copy2:
            try:
                baby = virus.reproduce(density, self.drugs)
                self.viruses.append(baby)
            except NoChildException:
                pass
        return self.getTotalPop()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    """
    poptimes = [0]*(delay+150)
    resistpop = [0]*(delay+150)
    for i in range(numTrials):
        viruslist = []
        for i in range(numViruses):
        ...
        for timestep in range(delay):
            newpop = dude.update()
            resistpopstep = dude.getResistPop(['guttagonol'])
            poptimes[timestep] += float(newpop)
            resistpop[timestep] += float(resistpopstep)
        dude.addPrescription('guttagonol')
        for timestep in range(delay,delay+150):
            newpop1 = dude.update()
            resistpopstep1 = dude.getResistPop(['guttagonol'])
            poptimes[timestep] += float(newpop1)
            resistpop[timestep] += float(resistpopstep1)
    populations = [x/numTrials for x in poptimes]
    resistants = [x/numTrials for x in resistpop]
    pylab.plot(range(delay+150), populations, label = "Total Virus Population")
    pylab.plot(range(delay+150), resistants, label = "Resistant Virus Population")
